...
//            用（DIF-DEA）*2即为MACD柱状图。
    ...
    static void calculateChangeMA(List<KLineEntity> datas, List<KLineEntity> kLineData) {
        ...
        for (int i = 0; i < 4; i++) {
            KLineEntity kLineEntity = kLineData.get(i);
            ma5 += kLineEntity.getClosePrice();
        }

        for (int i = 0; i < 9; i++) {
            KLineEntity kLineEntity = kLineData.get(i);
            ma10 += kLineEntity.getClosePrice();
        }

        for (int i = 0; i < 19; i++) {
            KLineEntity kLineEntity = kLineData.get(i);
            ma20 += kLineEntity.getClosePrice();
        }

        cma5 = ma5 / 4;
        cma10 = ma10 / 9;
        cma20 = ma20 / 19;
        KLineEntity kLineEntity = datas.get(0);
        kLineEntity.MA5Price = (cma5 + kLineEntity.getClosePrice()) / 2;
        kLineEntity.MA10Price = (cma10 + kLineEntity.getClosePrice()) / 2;
        kLineEntity.MA20Price = (cma20 + kLineEntity.getClosePrice()) / 2;
        Collections.reverse(kLineData);
    }
    ...
    public static void calculate(List<KLineEntity> datas, List<KLineEntity> mKlineData) {
        calculateChangeMA(datas, mKlineData);
        calculateMACD(datas);
        calculateBOLL(datas);
        calculateRSI(datas);
        calculateKDJ(datas);
        calculateChangeVolumeMA(datas, mKlineData);
    }
    ...
    private static void calculateChangeVolumeMA(List<KLineEntity> entries, List<KLineEntity> mKlineData) {
//        Collections.reverse(entries);
        float volumeMa5 = 0;
        float volumeMa10 = 0;

        for (int i = 0; i < 4; i++) {
            KLineEntity kLineEntity = mKlineData.get(i);
            volumeMa5 += kLineEntity.getVolume();
        }
        KLineEntity kLineEntity1 = entries.get(0);
        kLineEntity1.MA5Volume = (volumeMa5+kLineEntity1.getVolume())/5.0f;

        for (int i = 0; i < 9; i++) {
            KLineEntity kLineEntity = mKlineData.get(i);
            volumeMa10 += kLineEntity.getVolume();
        }
        kLineEntity1.MA10Volume = (volumeMa10+kLineEntity1.getVolume())/10.0f;
