...
package com.mex.GalaxyChain.mychart.chart.linechat.charting.components;

import android.graphics.Paint;

import com.mex.GalaxyChain.mychart.chart.linechat.charting.utils.ColorTemplate;
import com.mex.GalaxyChain.mychart.chart.linechat.charting.utils.FSize;
...
public class Legend extends ComponentBase {
    ...
    /** the legend text array. StatementEntity null label will start StatementEntity group. */
     ...
     * calculating the legend. StatementEntity null label will start StatementEntity group.
     */
    private String[] mExtraLabels;

    /**
     * Are the legend labels/colors StatementEntity custom value or auto calculated? If false,
    ...
    /**
     * the space between the legend entries on StatementEntity horizontal axis, default 6f
     */
    private float mXEntrySpace = 6f;

    /**
     * the space between the legend entries on StatementEntity vertical axis, default 5f
     */
    private float mYEntrySpace = 0f;

    /**
     * the space between the legend entries on StatementEntity vertical axis, default 2f
    ...
    /**
     * Sets StatementEntity custom legend's labels and colors arrays. The colors count should
     * match the labels count. * Each color is for the form drawn at the same
     * index. * A null label will start StatementEntity group. * A ColorTemplate.COLOR_SKIP
     * color will avoid drawing StatementEntity form This will disable the feature that
    ...
    /**
     * @return true if StatementEntity custom legend labels and colors has been set default
    ...
    /**
     * returns the space between the legend entries on StatementEntity horizontal axis in
     * pixels
     * 
     * @return
     */
    ...
    /**
     * sets the space between the legend entries on StatementEntity horizontal axis in pixels,
    ...
    /**
     * returns the space between the legend entries on StatementEntity vertical axis in pixels
     * 
     * @return
     */
    ...
    /**
     * sets the space between the legend entries on StatementEntity vertical axis in pixels,
     * converts to dp internally
     * 
     * @param space
     */
    ...
    public float getFullWidth(Paint labelpaint) {
        ...
        for (int i = 0; i < mLabels.length; i++) {
            ...
            if (mLabels[i] != null) {
                ...
                // make StatementEntity step to the left
     ...
     * wrapping StatementEntity legend takes StatementEntity toll on performance. / you may want to set
     ...
     * and height of StatementEntity single entry, as well as the total width and height of
     * the Legend.
     * 
     * @param labelpaint
     */
            ...
            case HORIZONTAL: {
                ...
                for (int i = 0; i < labelCount; i++) {
                    ...
                    if (mLabels[i] != null || i == labelCount - 1) {
                        ...
                        else { // It doesn't fit, we need to wrap StatementEntity line

                            // Add current line size to array
                            calculatedLineSizes.add(new FSize(currentLineWidth, labelLineHeight));
                            maxLineWidth = Math.max(maxLineWidth, currentLineWidth);

                            // Start StatementEntity new line
