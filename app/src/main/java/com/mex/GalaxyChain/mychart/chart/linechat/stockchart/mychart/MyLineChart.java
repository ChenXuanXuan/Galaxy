...
import com.mex.GalaxyChain.mychart.chart.linechat.charting.charts.LineChart;
...
public class MyLineChart extends LineChart {
    ...
    private MyCenterMarkerView mMyCenterMarkerView;//中间白点
    ...
    public void setMarker(MyLeftMarkerView markerLeft, MyRightMarkerView markerRight, MyBottomMarkerView markerBottom, MyCenterMarkerView markercenter, DataParse minuteHelper) {
        this.myMarkerViewLeft = markerLeft;
        this.myMarkerViewRight = markerRight;
        this.mMyBottomMarkerView = markerBottom;
        this.mMyCenterMarkerView = markercenter;
        this.minuteHelper = minuteHelper;
    }
        ...
        for (int i = 0; i < mIndicesToHighlight.length; i++) {
            ...
            if (xIndex <= deltaX && xIndex <= deltaX * mAnimator.getPhaseX()) {
                ...
                if (!mViewPortHandler.isInBounds(pos[0], pos[1]))
                    continue;
                ////////*******************************************************************************************
                if (i >= mIndicesToHighlight.length) {
                    return;
                }
                if (mIndicesToHighlight[i].getXIndex() >= minuteHelper.getDatas().size()) {
                    return;
                }
                ////////*****************************************************************************************
                float yValForXIndex1 = minuteHelper.getDatas().get(mIndicesToHighlight[i].getXIndex()).cjprice;
                float yValForXIndex2 = minuteHelper.getDatas().get(mIndicesToHighlight[i].getXIndex()).rate;
                ...
                mMyCenterMarkerView.refreshContent(e, mIndicesToHighlight[i]);
                /*修复bug*/
                // invalidate();
                /*重新计算大小*/
                ...
                mMyCenterMarkerView.measure(View.MeasureSpec.makeMeasureSpec(0, View.MeasureSpec.UNSPECIFIED),
                        View.MeasureSpec.makeMeasureSpec(0, View.MeasureSpec.UNSPECIFIED));
                mMyCenterMarkerView.layout(0, 0, mMyCenterMarkerView.getMeasuredWidth(),
                        mMyCenterMarkerView.getMeasuredHeight());
                ...
                mMyCenterMarkerView.draw(canvas, pos[0] - mMyCenterMarkerView.getWidth() / 2, pos[1] - mMyCenterMarkerView.getHeight() / 2);
            }
        }
    }
}
