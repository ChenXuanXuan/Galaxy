...
public class GridUtils {
    ...
    public static void drawGrid(Canvas canvas, float width, float height) {
        ...
        p.setPathEffect(new DashPathEffect(
                new float[]{8, 8, 8, 8}, 1));
        p.setColor(NewColorUtil.COLOR_EFFECT_FENSHI_LINE);
        p.setAntiAlias(true);
        //横虚线
        canvas.drawLine(0, height * 3 / 4, width, height * 3 / 4, p);
        canvas.drawLine(0, height * 2 / 4, width, height * 2 / 4, p);//中间横虚线
        canvas.drawLine(0, height * 1 / 4, width, height * 1 / 4, p);
        //竖虚线
        canvas.drawLine(width * 3 / 4, 0, width * 3 / 4, height, p);
        canvas.drawLine(width * 2 / 4, 0, width * 2 / 4, height, p);//中间竖虚线
...
//        canvas.drawLine(width * 2 / 4, 0, width * 2 / 4, height, p);
    ...
    public static void drawIndexGrid(Canvas canvas, float y, float width, float height) {
        ...
        canvas.drawLine(width * 2 / 4, y, width * 2 / 4, y + height, p);//中间竖虚线
...
//        canvas.drawLine(width * 2 / 4, y, width * 2 / 4, y + height, p);
     ...
     * @param duration
     */
    public static void drawIrregularGrid(Canvas canvas, float width, float height, String duration, int length) {
        float[] lineX = NewLineUtil.getXByDuration(duration, width);
        if (canvas == null) {
            Log.w("DrawUtils", "canvas为空");
            return;
        }
        float xUnit = width / (float) length;
        canvas.drawColor(Color.WHITE);
        Paint p = new Paint();
        p.setStyle(Paint.Style.STROKE);
        p.setPathEffect(new DashPathEffect(
...
//        }
        ...
        canvas.drawLine(0.59f * length * xUnit, 0, 0.59f * length * xUnit, height, p);
        canvas.drawLine(0.82f * length * xUnit, 0, 0.82f * length * xUnit, height, p);
        p.reset();
        p.setColor(NewColorUtil.COLOR_EFFECT_FENSHI_LINE);
        //中间实线(横)
//        canvas.drawLine(0, height * 2 / 4, width, height * 2 / 4, p);
     ...
     * @param duration
     */
    public static void drawIrregularIndexGrid(Canvas canvas, float y, float width, float height, String duration, int length) {
        if (canvas == null) {
            Log.w("DrawUtils", "canvas为空");
            return;
        }
        ...
        Paint p = new Paint();
        p.setStyle(Paint.Style.STROKE);
        p.setPathEffect(new DashPathEffect(new float[]{8, 8, 8, 8}, 1));
        p.setColor(NewColorUtil.COLOR_EFFECT_FENSHI_LINE);
        p.setAntiAlias(true);
        //横虚线
        canvas.drawLine(0, y + height / 2, width, y + height / 2, p);
...
//        canvas.drawLine(lineX[2], y, lineX[2], y + height, );

        canvas.drawLine(0.59f * length* xUnit, y, 0.59f * length * xUnit, y + height, p);
        canvas.drawLine(0.82f * xUnit, y, 0.82f * xUnit, y + height, p);
//        if (lineX.length == 5) {
