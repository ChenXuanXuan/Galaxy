...
public class CrossView extends View {
    ...
    private void drawCrossLine(Canvas canvas) {
        ...
        Paint p = new Paint();
        p.setAntiAlias(true);
        p.setColor(NewColorUtil.COLOR_CROSS_LINE);
        p.setStrokeWidth(1f);
        ...
        if (isJunXian) {
            //均线的时候才画出圆点
            //画十字线和均线价格线交汇的圆
            canvas.drawCircle(bean.x, bean.y, 8, p);
            p.setColor(NewColorUtil.COLOR_SMA_LINE);
            canvas.drawCircle(bean.x, bean.y2, 8, p);
        }else {
            canvas.drawCircle(bean.x, bean.y, 6, p);
            p.setColor(NewColorUtil.COLOR_SMA_LINE);
        }
        p.setColor(Color.BLACK);
        p.setTextSize(32f);
        //1, 写价格(竖线靠左时，价格需要写到右边)
        drawPriceTextWithRect(canvas, bean.x, bean.y, NumberUtil.beautifulDouble(bean.price), p);
        //2, 写时间
    ...
    private void drawIndexTexts(Canvas canvas) {
        ...
        p.setTextSize(26f);
        float x = 0;
        float y = getHeight() * (ChartConstant.MAIN_SCALE + ChartConstant.TIME_SCALE) + 10;
        for (int i = 0; i < bean.indexText.length; i++) {
            p.setColor(bean.indexColor[i]);
            canvas.drawText(NumberUtil.beautifulDouble(bean.indexText[i], 2), x, y, p);
            x += NewLineUtil.getTextWidth(p, NumberUtil.beautifulDouble(bean.indexText[i], 2)) + 30;
        }
    ...
    private void drawTimeTextWithRect(Canvas canvas, float x, String time, Paint p) {
        ...
        float textWidth = NewLineUtil.getTextWidth(p, time) + 20;
        float y = getHeight() * ChartConstant.FENSHI_SCALE;
        Paint rp = new Paint();
    ...
    private void drawPriceTextWithRect(Canvas canvas, float x, float y, String text, Paint p) {
        float textWidth = NewLineUtil.getTextWidth(p, text) + 10;
        Paint rp = new Paint();
        rp.setColor(Color.WHITE);
        rp.setStyle(Paint.Style.FILL);
        rp.setStrokeWidth(2f);
