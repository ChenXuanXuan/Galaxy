...
public class LandMarketMainAct extends BaseActivity implements View.OnClickListener,
    ...
    private TextView mTv_lossAmount,mTv_shiyin_amount, tv_enable_amount, tv_deal_perrmbmargin, tv_sum_amount, tv_confirm_postOrder_makeMoreOrLoss;
    private HorizontalListView hListView;
	private CustomWheelView wheelView;
	private String value;
	private TextView makeMore,makeEmpty;
    private String interval;
    private ImageView im_in_k;
    private TextView tvCurrent, open, high, close, low, tv_downUp_rate, tv_downUp,tv_makeMore_newPrice,tv_makeLoss_newPrice;
...
//  mTv_shiyin_amount.setText(Math.abs(Double.valueOf(value))*2+"");//触发止盈
                    ...
                    public void onNext(PostOrderBean postOrderBean) {
                        ...
                        if (code == 200) {
                           // ToastUtils.showTextInMiddle("下单成功");
                            UIHelper.jumptoMainActivity(LandMarketMainAct.this, "");//下单成功,跳到对应的已登陆持仓界面
    ...
    private void initDialog(int bsType, View inflate) {
        ...
        if (numEntityList != null && numEntityList.size() > 0)
        {
            ...
            priseAdapter = new PriseAdapter(LandMarketMainAct.this);
                    ...
                    .subscribe(new Subscriber<OrderBuyBean>() {
                        ...
                        public void onNext(OrderBuyBean orderBuyBean) {
                            ...
                            if (orderBuyBean.getCode() == 200) {
                                ...
                                mTv_lossAmount.setText("-" + DecimalFormatUtils.getDecimal(mPer_stoploss_amount * mDefault_handNum_one * mDefautl_beishu_one,2)); //(负)默认触发止损金额(默认1手1倍)
    ...
    private void showWheelDialog(List<MultiplBean> multiplBeanList) {
        ...
        View inflate = LayoutInflater.from(getActivity()).inflate(R.layout.layout_dialog_wheel_heng, null);
        ...
        // lp.height = WindowManager.LayoutParams.WRAP_CONTENT;
        lp.width = (int) (display.getWidth() * 0.5);// 高度设置为屏幕的0.5
        lp.height = (int) (display.getHeight() * 0.8); // 宽度设置为屏幕的0.65
        lp.y = 100; // 对话框居下偏移150 新位置Y坐标
    ...
    private void initWheelDialog(View inflate, List<MultiplBean> multiplBeanList) {//multiplBeanList 倍数集合
        ...
        for (MultiplBean multiplBean : multiplBeanList) { //便利倍数集合
            ...
            if (!isEmpty(mSelectedHandSum) && mSelectedHandSum != 0) {
                ...
                if (!isEmpty(mPer_stoploss_amount)) {
                    String sum_stoplossAmountWithBeiShou = "-" + DecimalFormatUtils.getDecimal(mPer_stoploss_amount * beishu * mSelectedHandSum*1.0,2);//(负)(总)触发止损金额=(负)单价止损金额*?手*?倍
        ...
        wheelView.setOnWheelViewListener(new CustomWheelView.OnWheelViewListener() {
            ...
            public void onSelected(int selectedIndex, String item) {
                ...
                tv_touch_StopLoss_amount.setText(DecimalFormatUtils.getDecimal(Double.valueOf(item),2));//(负)(总)触发止损金额(带倍数手数)=item=value
                tv_touch_StopProfi_amount.setText(   DecimalFormatUtils.getDecimal(Math.abs(Double.valueOf(item))*2.0,2)   ); //触发止盈金额=(正)(总)触发止损金额(带倍数手数)*2
                if (!isEmpty(mStoplossratio) && mStoplossratio != 0.0) {
                    tv_occupy_perrmbmargin.setText(DecimalFormatUtils.getDecimal(Math.abs(Double.valueOf(item))/mStoplossratio,2));//占用保证金=(正)触发止损金额/止损率
        ...
        inflate.findViewById(R.id.confirm).setOnClickListener(new View.OnClickListener()
        {
            ...
            public void onClick(View v) {
                ...
                if (!isEmpty(value)) {
                    mTv_lossAmount.setText(DecimalFormatUtils.getDecimal(Double.valueOf(value),2));//(负)(总价)止损金额
	...
	private void showPopu(){
		if (pwMyPopWindow.isShowing()) {
			...
			//int xpos = getActivity().getWindowManager().getDefaultDisplay().getWidth() - pwMyPopWindow.getWidth() +100;
	...
	private void initView() {
        ...
        tv_makeMore_newPrice = (TextView) findViewById(R.id.tv_makeMore_newPrice);//做多按钮的最新价
        tv_makeLoss_newPrice = (TextView) findViewById(R.id.tv_makeLoss_newPrice);//做空按钮的最新价
    ...
    private void initOthreView() {
         showSymbolInfoFromUserGolbal();
        ((TextView) findViewById(R.id.tv_symbolName_heng)).setText(symbolName);    //横屏标题
	...
	//	makeMore.setOnClickListener(this);
        ...
        im_in_k = (ImageView)findViewById(R.id.im_in_k);
        im_in_k.setOnClickListener(this);
        initPopu();
	}
    ...
    private void showSymbolInfoFromUserGolbal() {
