...
import com.mex.GalaxyChain.common.view.GlideCircleTransform;

/**
 * Created by LSJ on 16/3/18.
 * description：Glide简单封装
 */
...
public class GlideUtils {
    ...
    public static void loadImageView(Context mContext, String path, ImageView mImageView) {
        Glide.with(mContext).load(path).skipMemoryCache(false).diskCacheStrategy(DiskCacheStrategy.ALL).into(mImageView);
    }
    ...
    public static void loadCircleImageView(Context mContext, String path, ImageView mImageView) {
        Glide.with(mContext).load(path).skipMemoryCache(false).diskCacheStrategy(DiskCacheStrategy.ALL).transform(new GlideCircleTransform(mContext)).into(mImageView);
    }
